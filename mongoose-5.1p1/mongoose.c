...
#define MAX_SSL_ERRBUF  256
static char *ssl_errbuf[MAX_SSL_ERRBUF];
  ...
  if (ind < 0) {
    ...
    if (ind == LISTENING_PORT) {
      ...
      if ((server->ssl_ctx = SSL_CTX_new(SSLv23_server_method())) == NULL) {
        ...
        unsigned long e;
        int msglen;
        strcpy(ssl_errbuf, "Cannot load PEM file");
        msglen = strlen(ssl_errbuf);
#ifdef USE_OPENSSL
        while (e = ERR_get_error()) {
            const char *msg = ERR_error_string(e,NULL);
            if (msglen + strlen(msg) < MAX_SSL_ERRBUF)
                strcat(ssl_errbuf, msg); 
        }
#endif
        error_msg = ssl_errbuf;
      } else {
        SSL_CTX_use_certificate_chain_file(server->ssl_ctx, value);
        SSL_CTX_set_cipher_list(server->ssl_ctx,"ALL:!EXPORT:!EXPORT40:!EXPORT56:!aNULL:!LOW:!RC4:!CHACHA20:@STRENGTH");
      }
#endif
    }
  }
